'''Derived from auth_tkt.py from repoze.who.plugins and ckanext-dgu
DONL modifications:
...
class DONLAuthTktCookiePlugin(AuthTktCookiePlugin):
    # IIdentifier
    def remember(self, environ, identity):
        from pylons import config
        if 'donl_drupal_auth' in config['ckan.plugins']:
            ...
            if caller == ('drupal_auth.py', '_do_drupal_login'):
                ...
                return super(DONLAuthTktCookiePlugin, self).remember(environ, identity)
...
def make_plugin(secret=None,
    ...
    plugin = DONLAuthTktCookiePlugin(secret,
                                 cookie_name,
                                 _bool(secure),
                                 _bool(include_ip),
                                 timeout,
