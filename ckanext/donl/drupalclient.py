...
class DrupalClient(object):
    ...
    def get_xmlrpc_url(xmlrpc_settings=None):
        ...
        if xmlrpc_settings and xmlrpc_settings.get('xmlrpc_url'):
            ...
            if xmlrpc_settings:
                ...
                scheme = config.get('donl.xmlrpc_scheme', 'http')
                domain = config.get('donl.xmlrpc_domain')
                username = config.get('donl.xmlrpc_username')
                password = config.get('donl.xmlrpc_password')
            if not domain:
    ...
    def get_user_properties(self, user_id):
        ...
        log.info('Obtained Drupal user: %r', unicode(user)[:2000])
        return user
